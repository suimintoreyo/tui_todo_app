...
13. [リリースビルド（Nuitka）](#リリースビルドnuitka)
...
| `Esc` | フォーム・ダイアログを閉じる | フォーム / ダイアログ表示中 |

---

## リリースビルド（Nuitka）

Nuitka を使用して、Python 不要のスタンドアロン実行ファイルを生成できます。
...
```bash
# 1. ビルド用依存パッケージをインストール
pip install -r release/requirements-build.txt

# 2. ビルド実行（standalone モード）
python release/build.py
```

ビルド完了後、`release/dist/main.dist/` に配布用ディレクトリが生成されます。

### 実行

```bash
./release/dist/main.dist/main
```
...
| オプション | 説明 |
|-----------|------|
| `--onefile` | 単一実行ファイルとしてビルド |
| `--clean` | ビルド前に以前の成果物を削除 |

```bash
# 以前のビルドを削除してから 1 ファイル形式でビルド
python release/build.py --clean --onefile
```
...
| モード | 出力 | データ永続化 | 配布方法 |
|--------|------|-------------|---------|
| standalone（デフォルト） | ディレクトリ一式 | そのまま動作 | ディレクトリごと ZIP 等で配布 |
| onefile | 単一バイナリ | 実行ファイルと同階層に `data/` を配置 | バイナリ単体を配布 |

> **注意:** onefile モードでは `data/` がバイナリに内包されるため、スケジュールを永続保存するには実行ファイルと同じディレクトリに `data/` フォルダを別途配置してください。
